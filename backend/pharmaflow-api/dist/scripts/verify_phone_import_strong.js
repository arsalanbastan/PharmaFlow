...
function readAssignments() {
    ...
    const assignments = JSON.parse((0, node_fs_1.readFileSync)(ASSIGNMENTS_PATH, 'utf8'));
...
function normalizedImageInfo(value) {
    if (value === null) {
        return null;
    }
    return {
        length: value.length,
        sha256: (0, node_crypto_1.createHash)('sha256').update(value).digest('hex'),
    };
}
function addFieldMismatch(mismatches, counters, mismatch) {
...
async function main() {
    ...
    const duplicateExpectedUuidsTotal = duplicateExpectedCompanyUuids.length +
    ...
    (0, node_fs_1.writeFileSync)(REPORT_PATH, JSON.stringify(report, null, 2), 'utf8');
    if (passed) {
...
//# sourceMappingURL=verify_phone_import_strong.js.map