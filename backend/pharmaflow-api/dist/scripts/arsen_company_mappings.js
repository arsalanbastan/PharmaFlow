...
async function inspectCurrentState() {
    ...
    const existingByArsenId = new Map(existingMappings.map((item) => [item.arsenBusinessPartnerId, item]));
    ...
    const toCreate = mappings.filter((item) => !existingByArsenId.has(item.arsenBusinessPartnerId));
...
async function applyMappings(state) {
    ...
    if (state.deletedCompanies.length > 0) {
        throw new Error(`Mapped PharmaFlow companies are soft-deleted: ${state.deletedCompanies.map((item) => item.id).join(', ')}`);
    }
    if (state.conflicts.length > 0) {
        throw new Error(`Existing Arsen mappings point to different companies: ${state.conflicts.map((item) => item.arsenBusinessPartnerId).join(', ')}`);
    }
    await prisma.$transaction(async (tx) => {
        for (const item of mappings) {
            ...
            if (existing.companyId !== item.companyId) {
                throw new Error(`Refusing to remap Arsen BusinessPartnerID ${item.arsenBusinessPartnerId} from ${existing.companyId} to ${item.companyId}.`);
            }
...
//# sourceMappingURL=arsen_company_mappings.js.map