...
const order_similarity_1 = require("./order-similarity");
...
let OrdersService = class OrdersService {
    ...
    auditContext;
    pushOutbox;
    photoCleanupTimer = null;
    photoCleanupRetryRunning = false;
    canceledOrderExpiryRunning = false;
    constructor(prisma, auditLog, photoStorage, auditContext, pushOutbox) {
        this.prisma = prisma;
    ...
    async checkDuplicate(dto) {
        ...
        const matches = await this.findDuplicateMatches(dto.category, normalizedItemText);
        return {
    ...
    async create(dto) {
        ...
        const duplicateMatches = await this.findDuplicateMatches(dto.category, normalizedItemText);
        const data = {
    ...
    async preparePhotoUpload(id, dto) {
        const actor = this.requireAuthenticatedActor();
        const order = await this.requireOrder(id);
        this.assertCanAccessOrder(order, actor);
        if (order.status !== 'PENDING') {
            throw new common_1.BadRequestException('Photo can only be attached while the order is PENDING.');
        }
    ...
    async confirmPhotoUpload(id, dto) {
        const actor = this.requireAuthenticatedActor();
        const order = await this.requireOrder(id);
        this.assertCanAccessOrder(order, actor);
        if (order.status !== 'PENDING') {
    ...
    async uploadWebPhoto(id, dto) {
        ...
        if (bytes.length < 3 ||
            ...
            bytes[1] !== 0xd8 ||
            bytes[2] !== 0xff) {
            throw new common_1.BadRequestException('Order photo must be a valid JPEG file.');
        }
        const actualSha256 = (0, node_crypto_1.createHash)('sha256').update(bytes).digest('hex');
        if (actualSha256 !== dto.sha256.toLowerCase()) {
            throw new common_1.BadRequestException('Order photo SHA256 does not match.');
        }
    ...
    async attachPhotoMetadata(input) {
        ...
        try {
            return await this.prisma.$transaction(async (tx) => {
                ...
                if (changed.count !== 1) {
                    throw new common_1.BadRequestException('Order is no longer PENDING; photo was not attached.');
                }
    ...
    async createPhotoDownload(id) {
        const actor = this.requireAuthenticatedActor();
        const order = await this.requireOrder(id);
        this.assertCanAccessOrder(order, actor);
        if (order.status !== 'PENDING') {
            throw new common_1.NotFoundException('Order photo not found.');
        }
    ...
    async findAll(query) {
        ...
        if (actor.role === 'STAFF' &&
            ...
            throw new common_1.ForbiddenException('Staff can only list active PENDING or ORDERED requests.');
        }
        const statusWhere = actor.role === 'STAFF'
            ? status == null
                ? {
                    status: {
    ...
    async findChanges(query) {
        ...
        const cursorWhere = updatedAfter != null && afterId != null
            ? {
                OR: [
                    {
                        updatedAt: {
    ...
    async updateCategory(id, dto) {
        ...
        const duplicateMatches = await this.findDuplicateMatches(dto.category, existing.normalizedItemText);
    ...
    async updatePending(id, dto) {
        this.requireAuthenticatedActor();
        const existing = await this.requireOrder(id);
        if (existing.status !== 'PENDING') {
            throw new common_1.BadRequestException('Only PENDING orders can be edited.');
        }
        const normalizedItemText = this.normalizeItemText(dto.itemText);
        const duplicateMatches = await this.findDuplicateMatches(dto.category, normalizedItemText);
        const possibleDuplicate = duplicateMatches.some((match) => match.id !== id);
        return this.prisma.$transaction(async (tx) => {
            ...
            if (changed.count !== 1) {
                throw new common_1.BadRequestException('Order status changed before edit. Reload and try again.');
            }
    ...
    async assign(id, dto) {
        ...
        const existing = await this.requireOrder(id);
        if (existing.status !== 'PENDING') {
            throw new common_1.BadRequestException('Only PENDING orders can be assigned to a company.');
        }
        ...
        const updated = await this.prisma.$transaction(async (tx) => {
            ...
            if (changed.count !== 1) {
                throw new common_1.BadRequestException('Order status changed before assignment. Reload and try again.');
            }
            const current = await tx.orderRequest.findUnique({
                where: {
                    id,
    ...
    async returnToPending(id) {
        ...
        if (existing.status !== 'ORDERED') {
            throw new common_1.BadRequestException('Only ORDERED orders can be returned to PENDING.');
        }
        return this.prisma.$transaction(async (tx) => {
            const changed = await tx.orderRequest.updateMany({
                where: {
            ...
            if (changed.count !== 1) {
                throw new common_1.BadRequestException('Order status changed before it could be returned. Reload and try again.');
            }
            const current = await tx.orderRequest.findUnique({
                where: {
                    id,
    ...
    async receive(id) {
        ...
        const existing = await this.requireOrder(id);
        if (existing.status !== 'ORDERED') {
            throw new common_1.BadRequestException('Only ORDERED orders can be received.');
        }
        const now = new Date();
        const updated = await this.prisma.$transaction(async (tx) => {
            const changed = await tx.orderRequest.updateMany({
            ...
            if (changed.count !== 1) {
                throw new common_1.BadRequestException('Order status changed before receipt confirmation. Reload and try again.');
            }
            const current = await tx.orderRequest.findUnique({
                where: {
                    id,
    ...
    async cancel(id) {
        ...
        if (existing.status !== 'PENDING' && existing.status !== 'ORDERED') {
            throw new common_1.BadRequestException('Only PENDING or ORDERED orders can be canceled.');
        }
        const now = new Date();
        const updated = await this.prisma.$transaction(async (tx) => {
            const changed = await tx.orderRequest.updateMany({
            ...
            if (changed.count !== 1) {
                throw new common_1.BadRequestException('Order status changed before cancellation. Reload and try again.');
            }
            const current = await tx.orderRequest.findUnique({
                where: {
                    id,
    ...
    async restoreCanceled(id) {
        const actor = this.requireAuthenticatedActor();
        this.assertManager(actor);
        const existing = await this.requireOrder(id);
        if (existing.status !== 'CANCELED') {
            throw new common_1.BadRequestException('Only CANCELED orders can be restored.');
        }
        if (existing.canceledAt == null) {
            throw new common_1.BadRequestException('Canceled order has no cancellation timestamp.');
        }
        const now = new Date();
        if (now.getTime() - existing.canceledAt.getTime() >=
            CANCELED_ORDER_RETENTION_MS) {
            throw new common_1.BadRequestException('Canceled order restore window has expired.');
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (changed.count !== 1) {
                throw new common_1.BadRequestException('Order status changed before restore. Reload and try again.');
            }
            const current = await tx.orderRequest.findUnique({
                where: {
                    id,
    ...
    async removePending(id) {
        ...
        const updated = await this.prisma.$transaction(async (tx) => {
            ...
            if (changed.count !== 1) {
                throw new common_1.BadRequestException('Order status changed before deletion. Reload and try again.');
            }
            const current = await tx.orderRequest.findUnique({
                where: {
                    id,
    ...
    async expireCanceledOrdersBatch(now = new Date()) {
        if (this.canceledOrderExpiryRunning) {
            return 0;
        }
        this.canceledOrderExpiryRunning = true;
        try {
            const cutoff = new Date(now.getTime() - CANCELED_ORDER_RETENTION_MS);
            ...
            for (const existing of stale) {
                try {
                    ...
                    console.warn('[Orders] Automatic expiry failed for canceled order ' +
                        existing.id +
                        '.');
                }
            }
    ...
    async tryCleanupPhoto(orderId) {
        try {
            ...
            console.warn('[Orders] Best-effort photo cleanup failed for order ' + orderId + '.');
            return false;
        }
    }
    async retryPhotoCleanupBatch() {
    ...
    async deleteStorageObjectBestEffort(storageKey) {
        try {
            await this.photoStorage.deleteObject(storageKey);
        }
        catch {
            console.warn('[Orders] Best-effort cleanup of an unattached photo object failed.');
        }
    }
    ...
    async findDuplicateMatches(category, normalizedItemText) {
        ...
        return candidates
            .filter((candidate) => ACTIVE_DUPLICATE_STATUSES.has(candidate.status) &&
            (0, order_similarity_1.isOrderTextSimilar)(normalizedItemText, candidate.itemText))
            .map((candidate) => ({
            candidate,
            similarityScore: (0, order_similarity_1.scoreOrderTextSimilarity)(normalizedItemText, candidate.itemText),
        }))
    ...
    normalizeItemText(raw) {
        const normalized = (0, order_similarity_1.normalizeOrderSearchText)(raw);
        if (normalized.length === 0) {
            throw new common_1.BadRequestException('itemText cannot be empty after normalization.');
        }
        return normalized;
    }
    requireAuthenticatedActor() {
        ...
        if (context == null ||
            ...
            throw new common_1.UnauthorizedException('A verified authenticated user is required for Orders.');
        }
        return {
    ...
    assertManager(actor) {
        if (actor.role !== 'MANAGER') {
            throw new common_1.ForbiddenException('Only a MANAGER can perform this operation.');
        }
    }
    assertCanAccessOrder(order, actor) {
        ...
        if (order.requestedByUserId !== actor.userId) {
            throw new common_1.ForbiddenException('This order belongs to another user.');
        }
    }
    trimOrNull(raw) {
        if (raw == null) {
    ...
    normalizeOptionalCategory(raw) {
        const value = raw?.trim().toUpperCase();
        if (!value) {
            return undefined;
        }
        if (!VALID_CATEGORIES.has(value)) {
            throw new common_1.BadRequestException('Invalid order category.');
        }
        return value;
    }
...
exports.OrdersService = OrdersService = __decorate([
    ...
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        ...
        order_photo_storage_service_1.OrderPhotoStorageService,
        audit_context_service_1.AuditContextService,
        push_outbox_service_1.PushOutboxService])
], OrdersService);
