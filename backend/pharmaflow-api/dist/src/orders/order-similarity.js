...
exports.normalizeOrderSearchText = normalizeOrderSearchText;
exports.scoreOrderTextSimilarity = scoreOrderTextSimilarity;
exports.isOrderTextSimilar = isOrderTextSimilar;
const DIGIT_MAP = {
    '۰': '0',
    '۱': '1',
    '۲': '2',
...
function normalizeOrderSearchText(raw) {
    return raw
        .normalize('NFKC')
        .trim()
        .toLowerCase()
...
function scoreOrderTextSimilarity(queryText, candidateText) {
...
function isOrderTextSimilar(queryText, candidateText) {
    const normalizedQuery = normalizeOrderSearchText(queryText);
    const compactQuery = normalizedQuery.replace(/\s/g, '');
    if (compactQuery.length < 2) {
        return false;
    }
    return (scoreOrderTextSimilarity(normalizedQuery, candidateText) >= MIN_FUZZY_SCORE);
}
function weightedTokenSimilarity(query, candidate) {
...
function tokenSimilarity(left, right) {
    ...
    return Math.max(normalizedDamerauSimilarity(left, right), diceCoefficient(left, right));
}
function normalizedDamerauSimilarity(left, right) {
...
//# sourceMappingURL=order-similarity.js.map