...
let CatalogService = class CatalogService {
    ...
    async findAll(filters = {}) {
        ...
        if (search == null) {
            return this.findWithoutSearch(baseWhere, requestedPage, pageSize);
        }
        return this.findWithSmartSearch(baseWhere, search, requestedPage, pageSize);
    }
    async findOne(id) {
        const normalizedId = id.trim();
        if (!UUID_PATTERN.test(normalizedId)) {
            throw new common_1.BadRequestException('Invalid catalog item id.');
        }
        ...
        return {
            ...
            salesPrice: this.decimalString(item.salesPrice),
            lastPurchasePrice: this.decimalString(item.lastPurchasePrice),
            isActive: item.isActive,
            description: item.description,
            importedAt: item.importedAt.toISOString(),
            sourceSyncedAt: item.sourceSyncedAt.toISOString(),
        };
    }
    async findWithoutSearch(where, requestedPage, pageSize) {
        const totalCount = await this.prisma.arsenCatalogItem.count({
            where,
        });
        const totalPages = Math.max(1, Math.ceil(totalCount / pageSize));
        const page = Math.min(requestedPage, totalPages);
        const skip = (page - 1) * pageSize;
        const rows = await this.prisma.arsenCatalogItem.findMany({
            where,
            select: {
                id: true,
                ingestSequence: true,
    ...
    async findWithSmartSearch(baseWhere, search, requestedPage, pageSize) {
        const where = {
            ...baseWhere,
            AND: search.tokens.map((token) => this.buildTokenCondition(token)),
        };
        const rows = await this.prisma.arsenCatalogItem.findMany({
            where,
            select: {
        ...
        const ranked = rows
            .filter((row) => this.matchesAllTokens(row, search.tokens))
            .map((row) => ({
            row,
        ...
        return {
            items: pageRows.map(({ row }) => this.serializeSummary(row)),
            page,
            pageSize,
            totalCount,
            totalPages,
    ...
    serializeSummary(row) {
        return {
            ...
            lastPurchasePrice: this.decimalString(row.lastPurchasePrice),
    ...
    buildBaseWhere(filters) {
        ...
        if (category != null &&
            ...
            if (category !== 'DRUG' &&
                category !== 'GOODS') {
                throw new common_1.BadRequestException('category must be DRUG or GOODS.');
            }
            where.category = category;
        }
        const active = filters.active?.trim().toUpperCase();
        if (active != null &&
            active.length > 0 &&
            active !== 'ALL') {
            if (active === 'ACTIVE') {
                ...
                throw new common_1.BadRequestException('active must be ACTIVE, INACTIVE, or ALL.');
            }
        }
        return where;
    }
    ...
    searchVariants(token) {
        ...
        for (const [from, to] of replacements) {
            for (const value of [...variants]) {
                variants.add(value.split(from).join(to));
            }
        }
        return [...variants];
    }
    matchesAllTokens(row, tokens) {
        const fields = this.normalizedSearchFields(row);
        return tokens.every((token) => fields.some((field) => field.text.includes(token)));
    }
    scoreRow(row, search) {
        const fields = this.normalizedSearchFields(row);
        let score = 0;
        const arsenId = row.arsenDrugId.toString();
        ...
        for (const field of fields) {
            ...
            else if (text.startsWith(search.normalized)) {
                score += 6500 + field.weight;
            }
            else if (text.includes(search.normalized)) {
                score += 5200 + field.weight;
            }
            const positions = search.tokens.map((token) => text.indexOf(token));
            if (positions.every((position) => position >= 0)) {
                score += 2600 + field.weight;
                const starts = positions.filter((position) => position >= 0);
                const ends = search.tokens.map((token, index) => positions[index] + token.length);
                const span = Math.max(...ends) -
                    Math.min(...starts);
                score += Math.max(0, 900 - span * 8);
            }
            ...
            score -= Math.min(250, Math.abs(text.length -
                search.normalized.length));
        }
        return score;
    }
    normalizedSearchFields(row) {
        return [
            {
                text: this.normalizeSearchText(row.genericName ?? ''),
                weight: 100,
            },
            {
                text: this.normalizeSearchText(row.persianName ?? ''),
                weight: 95,
            },
            {
                text: this.normalizeSearchText(row.persianBrandName ?? ''),
                weight: 85,
            },
            {
                text: this.normalizeSearchText(row.brandName ?? ''),
                weight: 80,
            },
            {
