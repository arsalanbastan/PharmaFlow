...
exports.ADMIN_DASHBOARD_RELEASE = void 0;
exports.invoicePaginationWindow = invoicePaginationWindow;
...
exports.hardDeleteForm = hardDeleteForm;
exports.layout = layout;
exports.ADMIN_DASHBOARD_RELEASE = 'invoice-multi-settlement-v9-20260917';
function invoicePaginationWindow(currentPage, totalPages, maxVisible = 10) {
    const safeTotalPages = Math.max(1, Math.trunc(totalPages));
    const safeCurrentPage = Math.min(Math.max(1, Math.trunc(currentPage)), safeTotalPages);
    const visibleCount = Math.max(1, Math.min(Math.trunc(maxVisible), safeTotalPages));
    let start = Math.max(1, safeCurrentPage - Math.floor((visibleCount - 1) / 2));
...
function statusBadge(value) {
    ...
    const cssClass = status === 'RECEIVED' || status === 'ACTIVE'
        ? 'ok'
...
function hardDeleteForm(action, csrfToken, label, disabledReason) {
    if (disabledReason) {
...
<meta name="pharmaflow-admin-release" content="${exports.ADMIN_DASHBOARD_RELEASE}">
...
.filters{display:flex;gap:8px;flex-wrap:wrap}.filters>*{min-width:150px}.invoice-filters,.catalog-filters{margin-bottom:14px;align-items:end}.catalog-filters .field{min-width:145px}.catalog-filters .search-field{min-width:310px;flex:1}.catalog-filters .sort-field{min-width:220px}.catalog-filters .page-size{min-width:130px}.catalog-stats{margin-bottom:16px}.catalog-stat{cursor:default}.catalog-table table{min-width:1700px}.catalog-description{white-space:pre-wrap;line-height:1.9}.invoice-filters .field{min-width:170px}.invoice-filters .field.company{min-width:230px}.invoice-filters .field.page-size{min-width:130px}.pagination{display:flex;align-items:center;gap:6px;justify-content:center;margin-top:14px;flex-wrap:wrap;direction:ltr}.pagination .page-link{min-width:40px;height:38px;padding:7px 10px;border-radius:9px;background:#edf1f5;color:var(--text);text-decoration:none;display:inline-flex;align-items:center;justify-content:center}.pagination .page-link:hover{background:var(--primary-soft);color:var(--primary)}.pagination .page-link.active{background:var(--primary);color:#fff;font-weight:800}.pagination .page-link.disabled{opacity:.4;cursor:not-allowed;pointer-events:none}.pagination .ellipsis{min-width:28px;text-align:center;color:var(--muted);font-weight:700}.pagination-summary{display:flex;justify-content:center;margin-top:9px}.table-meta{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:8px 0 14px}.invoice-selection-bar{position:sticky;top:74px;z-index:8;display:flex;gap:14px;align-items:center;flex-wrap:wrap;margin:10px 0;padding:13px 15px;border:1px solid #9fcbd3;border-radius:12px;background:#e8f6f8;box-shadow:var(--shadow)}.invoice-selection-bar[hidden]{display:none}.invoice-row.payment-paid td{background:#edf9f1;color:#37664b}.invoice-row.payment-partial td{background:#fff9ea}.invoice-select{width:20px;height:20px}.settlement-summary{max-width:1050px;margin-bottom:16px;padding:16px;background:#fff;border:1px solid var(--border);border-radius:14px}.settlement-summary>strong{display:block;font-size:20px;margin-bottom:12px}.settlement-summary table{min-width:600px}
...
<body>
<header><div class="header-inner"><div class="brand">PharmaFlow Manager Web</div><nav>${nav}</nav><div class="release">${exports.ADMIN_DASHBOARD_RELEASE}</div></div></header>
<main>${body}</main>
<script>
