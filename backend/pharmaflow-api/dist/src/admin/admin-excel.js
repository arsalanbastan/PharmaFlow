...
function zipArchive(entries) {
    ...
    for (const entry of entries) {
        ...
        const compressed = (0, node_zlib_1.deflateRawSync)(entry.data, { level: 6 });
...
function estimatedWidth(values) {
...
function buildXlsx(sheetName, headers, rows) {
    ...
    const normalizedRows = rows.map((row) => Array.from({ length: headers.length }, (_, index) => row[index] ?? null));
    ...
    const bodyRows = normalizedRows
        ...
        const cells = row
            .map((value, columnIndex) => cellXml(`${columnName(columnIndex)}${excelRow}`, value))
            .join('');
...
//# sourceMappingURL=admin-excel.js.map