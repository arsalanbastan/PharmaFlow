...
exports.buildPrintToPdfReport = buildPrintToPdfReport;
...
function buildPrintToPdfReport(title, headers, rows, subtitle = '') {
    if (headers.length === 0) {
        throw new Error('PDF report requires at least one column.');
    }
    const body = rows
        .map((row) => {
        const cells = Array.from({ length: headers.length }, (_, index) => `<td>${escapeHtml(text(row[index]))}</td>`).join('');
        return `<tr>${cells}</tr>`;
    })
        .join('');
