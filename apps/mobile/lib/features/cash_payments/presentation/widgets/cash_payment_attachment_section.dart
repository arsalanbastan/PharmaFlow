...
class CashPaymentAttachmentDraft {
  CashPaymentAttachmentDraft({
    required this.kind,
  ...
  Future<void> persist(int cashPaymentId) async {
    if (_persisted) {
      return;
    }
    ...
    if (!await source.exists()) {
      throw StateError(
        'Temporary attachment file is missing before payment save.',
      );
    }
    ...
    final destinationDirectory = Directory(
      p.join(
        ...
        'cash_payment_attachments',
        cashPaymentId.toString(),
      ),
    );
    ...
    try {
      final repository = LocalCashPaymentAttachmentRepository(
...
class CashPaymentAttachmentSection extends ConsumerStatefulWidget {
  const CashPaymentAttachmentSection({
    super.key,
  ...
  final ValueChanged<List<CashPaymentAttachmentDraft>>? onDraftsChanged;

  @override
  ConsumerState<CashPaymentAttachmentSection> createState() =>
      _CashPaymentAttachmentSectionState();
}

class _CashPaymentAttachmentSectionState
    extends ConsumerState<CashPaymentAttachmentSection> {
  static const int _maximumBytes = 25 * 1024 * 1024;

  final ImagePicker _imagePicker = ImagePicker();

  late final LocalCashPaymentAttachmentRepository _repository;

  List<CashPaymentAttachment> _attachments = const [];

  final List<CashPaymentAttachmentDraft> _drafts = [];
  ...
  void initState() {
    ...
    _repository = LocalCashPaymentAttachmentRepository(
      DatabaseService.instance,
    );
  ...
  @override
  void didUpdateWidget(CashPaymentAttachmentSection oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (oldWidget.cashPaymentId != widget.cashPaymentId) {
      _load();
    }
  }

  Future<void> _load() async {
    final paymentId = widget.cashPaymentId;
    ...
    try {
      final attachments = await _repository.findByCashPaymentId(paymentId);
      ...
      debugPrint('CashPaymentAttachmentSection._load failed: $error');
  ...
  Future<void> _addAttachment(CashPaymentAttachmentKind kind) async {
    if (_isBusy) {
      return;
    }

    if (kind == CashPaymentAttachmentKind.statement) {
      await _addStatementAttachments();
      return;
    }
    ...
    try {
      ...
      final message = widget.cashPaymentId == null
          ? 'رسید آماده است و همراه با ذخیره واریزی ثبت می‌شود.'
          : 'رسید ذخیره شد و در صف همگام‌سازی قرار گرفت.';
      ...
      debugPrint(
        'CashPaymentAttachmentSection._addAttachment failed: '
        '$error',
      );
      debugPrintStack(stackTrace: stackTrace);
  ...
  Future<void> _addStatementAttachments() async {
    ...
    try {
      ...
      if (widget.cashPaymentId != null && added > 0) {
        await _load();
      }
  ...
  Future<void> _persistPickedAttachment({
    required CashPaymentAttachmentKind kind,
    ...
    if (paymentId == null) {
      ...
      attachmentDirectory = Directory(
        p.join(temporaryDirectory.path, 'cash_payment_attachment_drafts'),
      );
    } else {
      final documentsDirectory = await getApplicationDocumentsDirectory();
      attachmentDirectory = Directory(
    ...
    try {
      ...
      if (paymentId == null) {
        final draft = CashPaymentAttachmentDraft(
          kind: kind,
        ...
        widget.onDraftsChanged?.call(
          List<CashPaymentAttachmentDraft>.unmodifiable(_drafts),
        );
        return;
      }
  ...
  Future<void> _shareAttachment(CashPaymentAttachment attachment) async {
    if (_isBusy) {
      return;
    }
    ...
    try {
      ...
      debugPrint(
        'CashPaymentAttachmentSection._shareAttachment failed: $error',
      );
      debugPrintStack(stackTrace: stackTrace);
  ...
  Future<void> _requestDelete(CashPaymentAttachment attachment) async {
    ...
    try {
      ...
      debugPrint(
        'CashPaymentAttachmentSection._requestDelete failed: '
        '$error',
      );
      debugPrintStack(stackTrace: stackTrace);
  ...
  Future<File> _ensureLocalAttachmentFile(
    CashPaymentAttachment attachment,
  ) async {
    ...
    final remoteRepository = RemoteCashPaymentAttachmentRepository(
    ...
    try {
      ...
      final attachmentDirectory = Directory(
        p.join(
          documentsDirectory.path,
          'cash_payment_attachments',
          attachment.cashPaymentId.toString(),
        ),
      );
  ...
  Future<void> _preview(CashPaymentAttachment attachment) async {
    if (_isBusy) {
      return;
    }
    ...
    try {
      ...
      debugPrint('CashPaymentAttachmentSection._preview failed: $error');
      debugPrintStack(stackTrace: stackTrace);
  ...
  String _kindLabel(CashPaymentAttachmentKind kind) {
    switch (kind) {
  ...
  String _statusLabel(CashPaymentAttachment attachment) {
    final localPath = attachment.localPath?.trim();
  ...
  Future<void> _shareDraft(CashPaymentAttachmentDraft draft) async {
  ...
  Future<void> _deleteDraft(CashPaymentAttachmentDraft draft) async {
  ...
  Widget _draftTile(CashPaymentAttachmentDraft draft) {
    final file = File(draft.localPath);
  ...
  Widget _attachmentTile(CashPaymentAttachment attachment) {
    final localPath = attachment.localPath?.trim();
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                OutlinedButton.icon(
                  onPressed: _isBusy
                      ? null
                      : () => _addAttachment(CashPaymentAttachmentKind.receipt),
                ...
                OutlinedButton.icon(
                  onPressed: _isBusy
                      ? null
                      : () =>
                            _addAttachment(CashPaymentAttachmentKind.statement),
            ...
            if (_isLoading) ...[
              ...
              const Text(
                'هنوز رسید یا صورتحسابی برای این واریزی ثبت نشده است.',
                textAlign: TextAlign.right,
              ),
            ] else ...[
              const SizedBox(height: 6),
