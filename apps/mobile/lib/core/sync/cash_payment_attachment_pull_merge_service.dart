...
class CashPaymentAttachmentPullMergeResult {
  const CashPaymentAttachmentPullMergeResult({
    ...
    required this.localChangesProtected,
  ...
  final int localChangesProtected;
...
class CashPaymentAttachmentPullMergeService {
  ...
  Future<CashPaymentAttachmentPullMergeResult> pullAndMerge() async {
    ...
    _databaseService.transaction((db) {
      ...
      for (final change in changes) {
        ...
        if (localId != null && _hasActiveLocalSyncWork(db, localId)) {
          // The local queue is the source of truth until its pending change is
          // pushed. Do not let an older server snapshot abort every other sync
          // phase or overwrite the local receipt file.
          protectedUuids.add(change.id);
          localChangesProtected += 1;
          continue;
        }
    ...
    return CashPaymentAttachmentPullMergeResult(
      ...
      localChangesProtected: localChangesProtected,
      cursorBefore: cursorBefore,
      cursorAfter: cursorAfter,
    );
  }
