...
import '../../data/mappers/cheque_attachment_mapper.dart';
...
class ChequeAttachmentPullMergeException implements Exception {
  const ChequeAttachmentPullMergeException(this.message);

  final String message;

  @override
  String toString() => 'ChequeAttachmentPullMergeException: $message';
}

class ChequeAttachmentPullBlockedByLocalChangesException
    extends ChequeAttachmentPullMergeException {
  const ChequeAttachmentPullBlockedByLocalChangesException({
    required this.localId,
    required this.serverUuid,
  }) : super(
         'Cheque attachment pull merge blocked because local '
...
class ChequeAttachmentMergeConflictException
    extends ChequeAttachmentPullMergeException {
  const ChequeAttachmentMergeConflictException(super.message);
}

class ChequeAttachmentPullMergeResult {
  const ChequeAttachmentPullMergeResult({
...
class ChequeAttachmentPullMergeService {
  factory ChequeAttachmentPullMergeService({
    required DatabaseService databaseService,
    required RemoteChequeAttachmentRepository remoteRepository,
    required SyncCursorRepository cursorRepository,
    int pageLimit = RemoteChequeAttachmentRepository.defaultChangesLimit,
  }) {
    return ChequeAttachmentPullMergeService._(
      databaseService,
      remoteRepository,
      cursorRepository,
      pageLimit,
  ...
  ChequeAttachmentPullMergeService._(
  ...
  final RemoteChequeAttachmentRepository _remoteRepository;
  final SyncCursorRepository _cursorRepository;

  final int pageLimit;

  Future<ChequeAttachmentPullMergeResult> pullAndMerge() async {
    ...
    final latestChangesByUuid = <String, RemoteChequeAttachmentRecord>{};

    while (true) {
      if (pagesFetched >= _maximumPagesPerRun) {
        throw const ChequeAttachmentPullMergeException(
          'Cheque attachment pull exceeded the maximum allowed '
          'page count.',
        );
      }
    ...
    if (latestChangesByUuid.isEmpty) {
      return ChequeAttachmentPullMergeResult(
    ...
    _databaseService.transaction((db) {
      ...
      for (final change in changes) {
        ...
        if (localId != null && _hasActiveLocalSyncWork(db, localId)) {
          throw ChequeAttachmentPullBlockedByLocalChangesException(
            localId: localId,
        ...
        final parentLocalId = _requireChequeLocalId(
          db,
          chequeUuid: change.chequeId,
        ...
        if (localId != null) {
          ...
          if (existingParentId != parentLocalId) {
            throw ChequeAttachmentMergeConflictException(
              'Attachment ${change.id} is linked to local cheque '
      ...
      for (final change in changes) {
        ...
        final values = <String, Object?>{
          ...
          'cheque_id': parentLocalId,
          'kind': ChequeAttachmentMapper.kindToWireValue(change.kind),
  ...
  void _validatePage({
    required RemoteChequeAttachmentChangesPage page,
    ...
    RemoteChequeAttachmentRecord? previousItem;
...
''',
    ...
    if (rows.length > 1) {
      throw ChequeAttachmentMergeConflictException(
        'Cheque server UUID $chequeUuid maps to multiple '
    ...
    if (rows.isEmpty) {
      throw ChequeAttachmentMergeConflictException(
        'Cheque server UUID $chequeUuid is missing locally '
...
''',
    ...
    return rows.first['cheque_id'] as int;
  }
