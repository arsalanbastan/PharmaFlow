...
void main() {
  const chequeUuid = '11111111-1111-4111-8111-111111111111';
  ...
  test('prepare PUT confirm follows backend attachment contract', () async {
    ...
    final apiHttpClient = MockClient((request) async {
      if (request.method == 'POST' &&
          request.url.path.endsWith('/cheque-attachments/prepare-upload')) {
        prepareCalled = true;
        ...
        expect(body['chequeId'], chequeUuid);
      ...
      if (request.method == 'POST' &&
          request.url.path.endsWith('/cheque-attachments/confirm')) {
        confirmCalled = true;
    ...
    final repository = RemoteChequeAttachmentRepository(
      ApiClient(
    ...
    expect(confirmed.kind, ChequeAttachmentKind.statement);
  ...
  test('list changes download and delete contracts decode correctly', () async {
    ...
    final apiHttpClient = MockClient((request) async {
      if (request.method == 'GET' &&
          request.url.path.endsWith('/cheque-attachments')) {
        listCalled = true;

        expect(request.url.queryParameters['chequeId'], chequeUuid);
      ...
      if (request.method == 'GET' &&
          request.url.path.endsWith('/cheque-attachments/changes')) {
        changesCalled = true;
    ...
    final list = await repository.getAll(chequeId: chequeUuid);
