...
void main() {
  ...
  late LocalChequeAttachmentRepository repository;
  ...
  setUp(() {
    ...
    repository = LocalChequeAttachmentRepository(DatabaseService.instance);

    queueRepository = SyncQueueRepository(DatabaseService.instance);

    _insertParentCheque(database, id: 1);
  });
  ...
  test('migration 13 creates cheque attachments table', () {
    final version =
...
FROM sqlite_master
WHERE type = 'table'
  AND name = 'cheque_attachments'
''');

    expect(table, isNotEmpty);

    final columns = database
        .select('PRAGMA table_info(cheque_attachments)')
  ...
  test(
    ...
    () async {
      ...
      final id = await repository.insert(
        ChequeAttachment(
          ...
          sha256:
              ...
              'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
          localPath: r'C:\temp\statement.jpg',
          createdAt: now,
          updatedAt: now,
        ),
      );
      ...
      expect(saved.kind, ChequeAttachmentKind.statement);

      expect(saved.chequeId, 1);

      expect(saved.mimeType, 'image/jpeg');

      expect(saved.fileSize, 220000);

      expect(saved.localPath, r'C:\temp\statement.jpg');
    ...
    final id = await repository.insert(
      ChequeAttachment(
        ...
        sha256:
            ...
            'cccccccccccccccccccccccccccccccc',
        localPath: r'C:\temp\statement2.jpg',
        createdAt: now,
        updatedAt: now,
      ),
    );
...
void _insertParentCheque(Database db, {required int id}) {
